...
*/
//两种情况，插入在连续递增数组中间，或者插入在最高点到最低点的跳跃处。
class Solution {
public:
    Node* insert(Node* head, int insertVal) {
        ...
        Node *p=head;
        while(p->next!=head){
            if((p->val<=insertVal)^(p->next->val>=insertVal)^(p->next->val>=p->val))//三项条件同时满足或只满足一项才有效。同时满足即插入在中间，
            //只满足第一或第二个条件即在跳跃点，并包含了插入元素为数组最大和最小值的情况。
                break;
            p=p->next;
        }
